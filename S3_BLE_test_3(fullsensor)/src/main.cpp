...
static volatile uint8_t g_realHr = 255;
static volatile uint8_t g_realSpo2 = 255;
...
static void taskVitals(void *pvParams) {
    ...
    for (int i = 0 ; i < bufferLength ; i++) {
        while (!particleSensor.available()) {
            if (gI2cMutex) xSemaphoreTake(gI2cMutex, portMAX_DELAY);
            particleSensor.check();
            if (gI2cMutex) xSemaphoreGive(gI2cMutex);
    ...
    while (true) {
        ...
        if (validHeartRate && validSpo2 && heartRate > 30 && heartRate < 200 && spo2 > 50 && spo2 <= 100) {
            g_realHr = (uint8_t)heartRate;
            g_realSpo2 = (uint8_t)spo2;
            if (++printCounter >= 4) {
                Serial.printf("[MAX30102] HR=%d bpm, SpO2=%d%%\n", g_realHr, g_realSpo2);
                printCounter = 0;
            }
        } else {
            // Check if finger is not present (IR value drops low)
            if (irBuffer[50] < 50000) {
                ...
                if (++printCounter >= 4) {
                    Serial.println("[MAX30102] Calculating...");
                    printCounter = 0;
                }
            }
        }
...
static void sendInstantVitals() {
    ...
    char buf[80];
    // App parses splitting by '|', a single value is just a list of 1 element.
...
void setup() {
    ...
    xTaskCreatePinnedToCore(taskVitals, "VITALS", 4096, nullptr, 1, nullptr, 0);
    ...
    NimBLEDevice::init("S3_BLE_test_3");
    ...
    setLedState(LED_IDLE);
    Serial.println("[BOOT] Tasks started. Advertising as \"S3_BLE_test_3\"");
}
