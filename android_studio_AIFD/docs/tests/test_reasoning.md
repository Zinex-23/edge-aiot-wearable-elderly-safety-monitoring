...
> **Cập nhật lần cuối:** 2026-05-18 — Đối chiếu log thực tế `samsung-SM-M205G-Android-10_2026-05-18_220943.logcat`
...
| **Unit** | UT_APP_LOGIC | 4 | 🔶 3/4 | Kiểm tra logic ứng dụng Android |
| **Unit** | UT_CLOUD_DB | 4 | ⏳ Pending | Kiểm tra lưu trữ và phân phối dữ liệu trên Cloud |
| **Integrated** | IT_BLE_SYNC | 5 | 🔶 4/5 | Kiểm tra giao tiếp BLE giữa thiết bị đeo và điện thoại |
...
| **Integrated** | IT_VITALS_PIPELINE | 4 | 🔶 1/4 | Kiểm tra luồng dữ liệu sinh hiệu từ cảm biến đến màn hình app |
| **Integrated** | IT_EDGE_ALERT | 3 | ⏳ Pending | Kiểm tra pipeline từ AI phát hiện ngã đến cảnh báo trên app |
| **Integrated** | IT_OFFLINE_FIRST | 3 | 🔶 2/3 | Kiểm tra đường khẩn cấp chính không cần internet |
...
| **Tổng** | | **65** | **30/65** | 20 firmware ✅ + 10 Android partial 🔶 |

> **Ghi chú ký hiệu:** ✅ = pass đầy đủ · 🔶 = pass một phần (có bằng chứng từ log) · ⏳ = chưa test
...
| **Unit** | UT_APP_LOGIC | 4 | 🔶 3/4 | Android app là giao diện người dùng cuối. **Log 2026-05-18 xác nhận:** App khởi động OK, AifdApplication+Timber hoạt động, VitalsStore restore từ SharedPrefs, 3 ViewModel bind Service thành công. **Chưa xác nhận:** parse BATCH/ALERT payload (không có packet trong log), test CALL_PHONE permission. |
...
| **Integrated** | IT_BLE_SYNC | 5 | 🔶 4/5 | **Log 2026-05-18 xác nhận:** Auto-connect via bonded device ✓, GATT Error 133 → full teardown → exponential backoff (2s→5s→10s→20s→60s) ✓, manual scan ✓, GATT cache refresh ✓, MTU negotiation (255/512) ✓, service UUID 4fafc201 discovered ✓, cả 2 characteristic subscribed tuần tự không race condition ✓, READY command gửi thành công ✓, BleForegroundService chạy liên tục periodic check 60s ✓. **Chưa xác nhận:** BATCH data nhận sau kết nối (log kết thúc ngay sau connect). |
...
| **Integrated** | IT_VITALS_PIPELINE | 4 | 🔶 1/4 | **Log 2026-05-18 xác nhận:** BLE connection + subscription thành công (điều kiện tiên quyết để pipeline hoạt động) ✓. **Chưa xác nhận:** BATCH packet parse → VitalsStore → UI update (log kết thúc ngay sau connect, không có packet BATCH nào được ghi nhận). Cần session dài hơn. |
...
| **Integrated** | IT_OFFLINE_FIRST | 3 | 🔶 2/3 | **Log 2026-05-18 xác nhận:** Toàn bộ pipeline BLE (bonded → auto-connect → subscribe → READY) hoạt động hoàn toàn không cần internet ✓. BleForegroundService duy trì kết nối 24/7 ✓. **Chưa xác nhận:** Cloud auto-sync khi có internet sau sự cố (cần trigger ALERT). |
