...
class AppStrings(private val currentLanguage: AppLanguage) {
    ...
    val history get() = if (vi) "Lịch sử" else "History"
    ...
    val deviceDisconnected get() = if (vi) "Thiết bị đã ngắt kết nối" else "Device Disconnected"
    val tapToReconnect get() = if (vi) "Chạm để kết nối lại" else "Tap to reconnect"
    ...
    val eventHistoryEmpty get() = if (vi) "Lịch sử sự kiện sẽ hiển thị tại đây" else "Your event history will appear here"
    ...
    val lowSpo2Warning get() = if (vi) "Giá trị dưới 95% có thể cho thấy mức oxy máu thấp." else "Values below 95% may indicate low blood oxygen levels."
    ...
    val fallDetected get() = if (vi) "Phát hiện ngã" else "Fall Detected"
    ...
    val okayPrompt get() = if (vi) "Bạn có ổn không? Vui lòng phản hồi." else "Are you okay? Please respond."
    ...
    val imSafe get() = if (vi) "Tôi an toàn" else "I'm Safe"
