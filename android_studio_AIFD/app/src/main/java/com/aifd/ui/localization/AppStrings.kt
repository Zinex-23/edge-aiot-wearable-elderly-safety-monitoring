...
class AppStrings(private val currentLanguage: AppLanguage) {
    ...
    val changePassword get()        = if (vi) "Đổi mật khẩu" else "Change Password"
    val changePasswordSubtitle get() = if (vi) "Cập nhật mật khẩu đăng nhập" else "Update your login password"
    val changePasswordTitle get()   = if (vi) "Đổi mật khẩu" else "Change Password"
    val currentPassword get()       = if (vi) "Mật khẩu hiện tại" else "Current Password"
    val newPassword get()           = if (vi) "Mật khẩu mới" else "New Password"
    val confirmNewPassword get()    = if (vi) "Xác nhận mật khẩu mới" else "Confirm New Password"
    val passwordMismatch get()      = if (vi) "Mật khẩu xác nhận không khớp" else "Passwords do not match"
    val wrongCurrentPassword get()  = if (vi) "Mật khẩu hiện tại không đúng" else "Current password is incorrect"
    val passwordChanged get()       = if (vi) "Đổi mật khẩu thành công" else "Password changed successfully"
    val confirm get()               = if (vi) "Xác nhận" else "Confirm"
    ...
    val clearData get() = if (vi) "Xóa dữ liệu sức khỏe" else "Clear Health Data"
    val clearDataTitle get() = if (vi) "Xóa toàn bộ dữ liệu?" else "Clear all data?"
    val clearDataMessage get() = if (vi) "Hành động này sẽ xóa toàn bộ dữ liệu nhịp tim và SpO2 đã lưu (live, 1h, 24h). Không thể hoàn tác." else "This will permanently delete all stored HR and SpO2 data (live, 1h, 24h). This cannot be undone."
    val clearDataConfirm get() = if (vi) "Xóa" else "Clear"
    ...
    val networkDisconnected get() = if (vi) "Mất kết nối mạng" else "Network Disconnected"
    ...
    val noChartData get() = if (vi) "Chưa có dữ liệu" else "No data yet"
    val noDataInPastHour get() = if (vi) "Chưa có dữ liệu trong 1 giờ qua" else "No data in the past hour"
    val waitingForReadings get() = if (vi) "Đang chờ dữ liệu từ thiết bị..." else "Waiting for readings from your device..."
    val noEventsTitle get() = if (vi) "Chưa có sự cố" else "No events yet"
    val noEventsSubtitle get() = if (vi) "Mọi cảnh báo sẽ hiện ở đây." else "Any future alerts will appear here."
    val dangerZone get() = if (vi) "Vùng nguy hiểm" else "Danger Zone"
