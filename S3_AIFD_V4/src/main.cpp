 ...
 *       peak_acc > 7.5g OR peak_gyro > 240 dps → FALL_WATCH → STILL_TIMING
...
static const float FALL_ACC_THRESHOLD   = 1.8f;    // lowered for easier threshold testing
static const float FALL_GYRO_THRESHOLD  = 120.0f;  // lowered for easier threshold testing
static const float ACTIVITY_ACC_THRESHOLD   = 1.2f;   // pre-fall movement gate
static const float ACTIVITY_GYRO_THRESHOLD  = 25.0f;  // pre-fall movement gate
...
static void handleBlink() {
    ...
    // Idle BLE indicators are visible only after the pre-activity gate is armed.
    if (st == LED_CONNECTED) {
    ...
    switch (fallDetectState) {
        case FDS_IDLE:
            if (thresholdFall) {
                ...
                Serial.printf("[FSM] IDLE -> FALL_WATCH (threshold %.2fg/%.1fdps, left=%d)\n",
                              peakAcc, peakGyr, fallWatchLeft);
            }
            break;
        ...
        case FDS_STILL_TIMING: {
            ...
            if (cancelActive) {
                ...
                stillnessTimerArmed = false;
                restorePreFallLedState();
                Serial.printf("[FSM] STILL_TIMING: strong activity (%.2fg/%.1fdps) -> IDLE\n",
                              peakAcc, peakGyr);
                break;
            }
